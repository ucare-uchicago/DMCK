	...
	public synchronized void proceedEachFile(String filename, Properties ev) {
		if (filename.startsWith("zkLE-")) {
			...
			if (dmck.isNodeOnline(sender) && dmck.isNodeOnline(recv)) {
				...
				LOG.debug("DMCK receives ZK LE event with hashId-" + hashId + " sender-" + sender + " recv-" + recv
						+ " state-" + state + " leader-" + leader + " zxid-" + zxid + " epoch-" + epoch + " filename-"
						+ filename);
				appendReceivedUpdates(
						"New Event: filename=" + filename + " sender=" + sender + " recv=" + recv + " state=" + state
								+ " leader=" + leader + " zxid=" + zxid + " epoch=" + epoch + " hashId=" + hashId);
			...
			if (dmck.isNodeOnline(sender) && dmck.isNodeOnline(recv)) {
				...
				LOG.debug("DMCK receives ZK Snapshot event with hashId-" + hashId + " sender-" + sender + " recv-"
						+ recv + " eventType-" + eventType + " filename-" + filename);
				appendReceivedUpdates("New Event: filename=" + filename + " sender=" + sender + " recv=" + recv
			...
			if (dmck.isNodeOnline(sender) && dmck.isNodeOnline(recv)) {
				...
				appendReceivedUpdates("New Event: filename=" + filename + " sender=" + sender + " recv=" + recv
						+ " eventType=" + eventType + " port=" + port + " hashId=" + hashId);
			...
			if (dmck.isNodeOnline(sender) && dmck.isNodeOnline(recv)) {
				...
				LOG.debug("DMCK receives ZK ZAB event with hashId-" + hashId + " sender-" + sender + " recv-" + recv
						+ " eventType-" + eventType + " key-" + key + " value-" + value + " port-" + port + " filename-"
						+ filename);
				appendReceivedUpdates("New Event: filename=" + filename + " sender=" + sender + " recv=" + recv
						+ " eventType=" + eventType + " key=" + key + " value=" + value + " port=" + port + " hashId="
						+ hashId);
			...
			if (dmck.isNodeOnline(nodeId)) {
				...
				appendReceivedUpdates("New Event: filename=" + filename + " nodeId=" + nodeId + " eventType="
			...
			appendReceivedUpdates(
					"New Update: filename=" + filename + " nodeId=" + sender + " state=" + state + " proposedLeader="
			...
			if (votesTable.isEmpty()) {
				...
				appendReceivedUpdates("New Update: filename=" + filename + " nodeId=" + sender + " votesTable="
						+ votesHash.toString());
			}
